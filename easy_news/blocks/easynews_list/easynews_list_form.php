...
<ul id="ccm-newslist-tabs" class="ccm-dialog-tabs">
	<li class="ccm-nav-active"><a id="ccm-newslist-tab-add" href="javascript:void(0);"><?php echo ($bID>0)? t('Edit') : t('Add') ?></a></li>
	<li class=""><a id="ccm-newslist-tab-preview"  href="javascript:void(0);"><?php echo t('Preview')?></a></li>
</ul>

<input type="hidden" name="pageListToolsDir" value="<?php echo $uh->getBlockTypeToolsURL($bt)?>/" />
<div id="ccm-newslistPane-add" class="ccm-newslistPane">
	<div class="ccm-block-field-group">
	  <h2><?php echo t('Number and Type of News')?></h2>
	  <?php echo t('Display')?>
	  <input type="text" name="num" value="<?php echo $num?>" style="width: 30px">
	  <?php echo t('news of type')?>
	  <?php 
	...
	<div class="ccm-block-field-group">
	  ...
	  <?php echo t('Display news that are located')?>:<br/>
	  <br/>
	  <div>
	...
	<div class="ccm-block-field-group">
	  <h2><?php echo t('Sort News')?></h2>
	  <?php echo t('News should appear')?>
	  <select name="orderBy">
		<option value="public_asc" <?php  if ($orderBy == 'public_asc') { ?> selected <?php  } ?>><?php echo t('in publication date order asc')?></option>
		<option value="public_desc" <?php  if ($orderBy == 'public_desc') { ?> selected <?php  } ?>><?php echo t('in publication date order desc')?></option>
	...
	<div class="ccm-block-field-group">
	  <h2><?php echo t('Provide RSS Feed')?></h2>
	   <input id="ccm-newslist-rssSelectorOn" type="radio" name="rss" class="rssSelector" value="1" <?php echo ($rss?"checked=\"checked\"":"")?>/> <?php echo t('Yes')?>   
	   &nbsp;&nbsp;
	...
	<style type="text/css">
	#ccm-newslist-truncateTxt.faintText{ color:#999; }
	<?php  if(truncateChars==0 && !$truncateSummaries) $truncateChars=128; ?>
	</style>
	<div class="ccm-block-field-group">
	   <h2><?php echo t('Truncate Summaries')?></h2>	  
	   <input id="ccm-newslist-truncateSummariesOn" name="truncateSummaries" type="checkbox" value="1" <?php echo ($truncateSummaries?"checked=\"checked\"":"")?> /> 
	   <span id="ccm-newslist-truncateTxt" <?php echo ($truncateSummaries?"":"class=\"faintText\"")?>>
	   		<?php echo t('Truncate descriptions after')?> 
			<input id="ccm-newslist-truncateChars" <?php echo ($truncateSummaries?"":"disabled=\"disabled\"")?> type="text" name="truncateChars" size="3" value="<?php echo intval($truncateChars)?>" /> 
...
<div id="ccm-newslistPane-preview" style="display:none" class="ccm-preview-pane ccm-newslistPane">
	<div id="newslist-preview-content"><?php echo t('Preview Pane')?></div>
</div>